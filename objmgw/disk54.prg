*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Source Module => C:\DEVELOP\OBJ\DISK54.PRG
*+
*+    Functions: Function AC_AGUDO()
*+               Function AC_CRASE()
*+               Function AC_TIL()
*+               Function AC_CIRC()
*+               Function Acentuar()
*+               Function liga_acento()
*+               Function Acento()
*+
*+    Reformatted by Click! 2.03 on Sep-13-2004 at  9:54 am
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function AC_AGUDO()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func AC_AGUDO

set key 39 to
ACENTUAR( "AGU" )
set key 39 to AC_AGUDO
retu

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function AC_CRASE()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func AC_CRASE

Acentuar( "CRA" )
retu

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function AC_TIL()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func AC_TIL

Acentuar( "TIL" )
retu

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function AC_CIRC()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func AC_CIRC

ACENTUAR( "CIR" )
retu

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function Acentuar()
*+
*+    Called from ( disk54.prg   )   1 - function ac_agudo()
*+                                   1 - function ac_crase()
*+                                   1 - function ac_til()
*+                                   1 - function ac_circ()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func Acentuar( cTIPO )

local cDOS
local cWIN
local cBUS
local nPOS
cBUS := { "A", "a", "E", "e", "I", "i", "O", "o", "U", "u", "C", "c" }
do case
case cTIPO = "CRA"  //Crase
   cDOS := CHARCNVMT( "·…ФЉЮЌг•л—Ђ‡" )
   cWIN := CHARCNVMT( "+а+иЭмТ=+щЗз" )
case cTIPO = "AGU"  //Agudo
   cDOS := CHARCNVMT( "µ ђ‚ЦЎаўйЈЂ‡" )
   cWIN := CHARCNVMT( "-б+й-нУу+ъЗз" )
case cTIPO = "CIR"  //Circunflexo
   cDOS := CHARCNVMT( "¶ѓТ€ЧЊв“к–Ђ‡" )
   cWIN := CHARCNVMT( "-в-к+оФфЭыЗз" )
case cTIPO = "TIL"  //Til
   cBUS := { "A", "a", "O", "o", "N", "n" }
   cDOS := CHARCNVMT( "ЗЖедҐ¤" )
   cWIN := CHARCNVMT( "г+iхСс" )
case cTIPO = "TRE"  //Trema
   cDOS := CHARCNVMT( "Ћ„У‰Ш‹™”љЃЂ‡" )
   cWIN := CHARCNVMT( "-д-лПпЦц_ьЗз" )
case cTIPO = "GRA"  //Grau
   cBUS := { "A", "a" }
   cDOS := CHARCNVMT( "Џ†" )
   cWIN := CHARCNVMT( "+е" )
endcase
xkey := inkey( 0 )
ykey := chr( xkey )
nPOS := ascan( cBUS, yKEY )
if nPOS > 0
   Kkey := if( ACENTUA, cDOS[ nPOS ], cWIN[ nPOS ] )
else
   kkey := ykey
endif
keyboard ( kkey )
retu

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function liga_acento()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func liga_acento

if ALERTX( "Ligar Acentua‡„o", { "Sim", "N„o" } ) = 1
   set key 39 to AC_AGUDO
   set key 94 to AC_CIRC
   set key 96 to AC_CRASE
   set key 126 to AC_TIL
   if ALERTX( "Acentua‡„o para Windows", { "Sim", "N„o" } ) = 1
      ACENTUA := .F.
   else
      ACENTUA := .T.
   endif
else
   set key 39 to
   set key 94 to
   set key 96 to
   set key 126 to
endif
retu

*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
*+    Function Acento()
*+
*+ЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭЭ
*+
func Acento( texto )

local ag   := chr( 8 ) + chr( 39 )
local ti   := chr( 8 ) + chr( 126 )
local cr   := chr( 8 ) + chr( 96 )
local ci   := chr( 8 ) + chr( 94 )
local ce   := chr( 8 ) + chr( 44 )
local sp   := 0
local X
local aORI := { " ", "‚", "Ў", "ў", "Ј", ;
                "µ", "ђ", "Ц", "а", "й", ;
                "ѓ", "€", "“", "…", "·", ;
                "‡", "Ђ", "¶", "Т", "в", ;
                "Ж", "„", "д", "”", "З", "Ћ", "™", "е" }
local aDES := { "a" + ag, "e" + ag, "i" + ag, "o" + ag, "u" + ag, ;
                "A" + ag, "E" + ag, "I" + ag, "O" + ag, "U" + ag, ;
                "a" + ci, "e" + ci, "o" + ci, "a" + cr, "A" + cr, ;
                "c" + ce, "C" + ce, "A" + ci, "E" + ci, "O" + ci, ;
                "a" + ti, "a" + ti, "o" + ti, "o" + ti, "A" + ti, "A" + ti, "O" + ti, "O" + ti }
if valtype( TEXTO ) # "C"
   retu TEXTO
endif
if type( "nTIPSPO" ) # "U"              //Nao passado tipo spoll
   retu TEXTO
endif
if nTIPSPO <> 2 .and. nTIPSPO <> 3 .and. nTIPSPO <> 4       //LPT1 LPT2 LPT3
   retu TEXTO
endif
for X := 1 to len( aORI )
   if at( aORI[ X ], texto ) > 0
      texto := strtran( texto, aORI[ X ], aDES[ X ] )
      sp ++
   endif
next
if sp > 0
   texto += space( sp )
endif
retu ( texto )

*+ EOF: DISK54.PRG
