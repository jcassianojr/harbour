...
CLSBOX(6,0,17,80)
   ...
   oPCAO(10,01," hb&NFeEmail ",78)   // N - Nova opÁ„o incluÌda
   nOPCAO := menu(,0)
restscreen(6,0,17,80,cTELA)   
...
// Instancia o objeto da classe hbnfeemail
   ...
   // Converte para min˙sculo e limpa espaÁos para garantir a comparaÁ„o automatizada
   cServerLower := AllTrim( Lower( cServerIP ) )

   // DESCOBERTA E ESCOPO AUTOM¡TICO ADADEQUADO AO SERVER DO INI
   DO CASE
      CASE "gmail" $ cServerLower
         // Se no INI estiver smtp.gmail.com ou similar
         // O mÈtodo UseGmail j· configura internamente a porta 465, SSL direto e associa o From ao usu·rio [cite: 11, 12, 13]
      ...
      CASE "office365" $ cServerLower .OR. "outlook" $ cServerLower
         ...
         // O mÈtodo UseMicrosoft365 configura os canais internos da Microsoft (Porta 587, TLS, SSL) [cite: 16, 17, 100]
         oEmail:UseMicrosoft365( cUser, cPass )

      CASE "brevo" $ cServerLower
         // Se no INI estiver smtp-relay.brevo.com
         // O mÈtodo UseBrevo aceita: cSmtpLogin, cSmtpKey, cFrom, nPorta [cite: 114]
         oEmail:UseBrevo( cUser, cPass, cFrom, nPorta ) 

      OTHERWISE
         // Servidor genÈrico do INI (Ex: mail.seudominio.com.br, smtp.terra.com.br, etc) [cite: 74]
         // Passa os par‚metros explÌcitos lidos do seu INI
         // Par‚metros: cServer, nPorta, cUsuario, cSenha, lSSL, lTLS 
         oEmail:UseSMTP( cServer, nPorta, cUser, cPass, (nPorta == 465), (nPorta == 587) )
   ENDCASE

   // Preenche os dados din‚micos recebidos nos par‚metros da funÁ„o
   oEmail:cSubject  := cASSUNTO
   oEmail:cMsgTexto := cCORPOMSG
   
   // Se cFrom estiver populado diferentemente do padr„o mapeado nos mÈtodos, forÁa o do INI [cite: 13, 16, 18]
   IF ! Empty( cFrom )
      oEmail:cFrom := cFrom
   ENDIF

   // Adiciona o destinat·rio (Supondo que vocÍ use uma vari·vel global ou passe no bloco)
   // oEmail:AddTo( cDESTINATARIO ) [cite: 107]

   // Adiciona o anexo passado no par‚metro se ele existir fisicamente
...
function printtoodt( cARQ, cFileToSave )
   ...
   oDoc:AddHeading( "RelatÛrio do Sistema", 1 )
   ...
   DO WHILE HB_FReadLine( nFileHandle, @cLinha ) == 0
      IF cLinha == "##page##"
         // Salto de p·gina opcional ou separador no ODT
         oDoc:AddParagraph( "----------------------------------------" )
      ELSE
...
FUNCTION filezebrapdf( cARQSPO )
   ...
   // 1. Define o nome do arquivo PDF de saÌda baseado no arquivo .TXT gerado
   cFILE := SubStr( cARQSPO, 1, At( ".", cARQSPO ) - 1 ) + ".pdf"
   
   // 2. LÍ o conte˙do ZPL bruto que o seu sistema j· gerou no disco
   ...
   // 3. Instancia a classe ZPL (o padr„o do mÈtodo New() j· È etiqueta 4x6 em 203dpi/8dpmm)
   oZPL := ZPLDocument():New()
   
   // 4. Injeta todo o cÛdigo ZPL no documento
   ...
   IF ! oZPL:SaveToPDF( cFILE )
      // Retorna vazio caso n„o haja internet ou a API falhe, evitando que o sistema tente abrir um PDF fantasma
      cFILE := "" 
   ENDIF
