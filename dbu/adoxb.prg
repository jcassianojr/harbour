...
*+--------------------------------------------------------------------
*+
Function ADODBCREATE(cDatabase,cUserName, cPassword, lEncrypt)
CreateAccessDatabase(cDatabase, cUserName, cPassword, lEncrypt)
...
LOCAL cTmp := ""
cTmp :=SqliteCreateTable( cTablename, aFields) //SqliteCreateTable( cTablename, aStruct, cTIPOSQL, lINDEX ,lPK,lINCSR)
ADO EXECUTE cTmp
/*
LOCAL cTmp := ""
...
LOCAL cTmp := ""
cTmp :=SqliteCreateTable( cTable, aFields) //SqliteCreateTable( cTablename, aStruct, cTIPOSQL, lINDEX ,lPK,lINCSR)
ADO EXECUTE cTmp
...
//geraconn(cCAMBASE,lPROVIDER,StrUsuario,StrSenha,StrServer,StrPort)
StrConnection :=geraconn(StrDatabase,,StrUsuario,StrSenha,StrServer,StrPort)
...
if StrDriver = "FIREBIRD" .or. cEXTENSAO == ".fgb" .or. cEXTENSAO == ".gdb" .or. cEXTENSAO == ".ib"   // ADOGDB
   ...
   StrConnection := "DRIVER="+DriverFirebird()+"; UID="+StrUsuario+"; PWD="+StrSenha+"; DBNAME="+StrDatabase
...
if nRecordSet > 0
   ...
   do case
   case cType = 'C' 
      xValor := if(empty(uVal),spac(nLong),uVal+spac(nLong - len(uVal)))
   case cType = 'D' 
      xValor := if(empty(uVal),ctod(''),uVal)
   case cType = 'N' 
      xValor := if(empty(uVal),0,uVal)
   case cType = 'L' 
      xValor := if(empty(uVal),.f.,uVal)
