...
WHILE .T.
   ...
   DO CASE
      ...
      CASE LASTKEY()=K_ENTER ; MDS('Exibindo  ') ; FMANUAL(1,ACHEI)
      CASE LASTKEY()=K_CTRL_ENTER ; MDS('Imprimindo') ; FMANUAL(2,ACHEI)
      OTHERWISE ; LOOP
   ENDCASE
ENDDO
RETU
...
ENDIF
DO CASE
	CASE LASTKEY()=K_CTRL_ENTER ; RETU 1
	CASE LASTKEY()=K_ESC ; RETU 0
	CASE LASTKEY()=K_CTRL_ENTER ; RETU 1
	CASE LASTKEY()=K_HOME   ; KEYBOARD CHR(K_CTRL_PGUP) &&HOME VIRA CTRL_PGUP
	CASE LASTKEY()=K_END    ; KEYBOARD CHR(K_CTRL_PGDN) &&END  VIRA CTRL_PGDN
	OTHERWISE ; RETU 2
ENDCASE
RETU .T.
...
DO CASE
	CASE LASTKEY()=K_HOME   ; KEYBOARD CHR(K_CTRL_PGUP) &&HOME VIRA CTRL_PGUPF
	CASE LASTKEY()=K_END    ; KEYBOARD CHR(K_CTRL_PGDN) &&END  VIRA CTRL_PGDN
ENDCASE
