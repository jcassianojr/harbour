...
WHILE .T.
   ...
   ENDIF
   DO CASE
   CASE OPR = 1 .AND. (TIP = 1 .OR. TIP = 4) 
      FOIIA()
   CASE OPR = 1 .AND. (TIP = 2 .OR. TIP = 3) 
      FOIIB()
   CASE OPR = 1 .AND. TIP = 5
      ...
      ELSE
         FOID1()
      ENDIF
   CASE OPR = 1 .AND. TIP = 6 
      FOIF1()
   CASE OPR = 2 .AND. (TIP = 1 .OR. TIP = 4) 
      MDT('Este resumo n„o necessita acumla‡„o')
   CASE OPR = 2 .AND. (TIP = 2 .OR. TIP = 3 .OR. TIP = 5 .OR. TIP = 6) 
      FOIIA()
   CASE OPR = 3 .AND. TIP = 1 
      FOIB(1)
   CASE OPR = 3 .AND. (TIP = 2 .OR. TIP = 4 .OR. TIP = 5 .OR. TIP = 6) 
      MDT('Este resumo n„o utiliza titulos')
   CASE OPR = 3 .AND. TIP = 3 
      FOIB(2)
   OTHERWISE 
      EXIT
   ENDCASE
