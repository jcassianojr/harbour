...
WHILE .T.
   ...
   ENDIF
   DO CASE
      CASE TIP=8.AND.ARQ=6   ; FOCG()
      CASE TIP=8.AND.ARQ=8   ; HELPDBF="FO07"    ; FOC2()
      CASE TIP=1.AND.ARQ#9.AND.ARQ#10 ; FOCA2(ARQ)
      CASE TIP=10                     ; FOCB(ARQ)
      CASE TIP=11                     ; FOCK(ARQ)
      OTHERWISE   ; FOC2()
   ENDCASE
ENDDO
RETU
...
WHILE .T.
   ...
   ENDIF
   DO CASE
      CASE TIP=1.AND.ARQ=10 ;  FOCA1(445,527,409,'PREMIO TRIBUTADO',KEY)
      CASE TIP=1.AND.ARQ=9 ;  FOCA1( 41,501,403,'ADIANTAMENTO SALARIAL',KEY)
      CASE TIP=2           ;  FOCC(ARQ,KEY,2)
      CASE TIP=3           ;  FOCC(ARQ,KEY,1)
      CASE TIP=4           ;  FOCD(ARQ,KEY)
      CASE TIP=5           ;  FOCF(ARQ,1,KEY)
      CASE TIP=6           ;  FOCF(ARQ,0,KEY)
      CASE TIP=7           ;  FOCC(ARQ,KEY,3)
      CASE TIP=8           ;  FOCJ(ARQ,KEY)
      CASE TIP=9           ;  FOCC(ARQ,KEY,4)
      CASE TIP=12          ;  FOCC(ARQ,KEY,5)
      OTHERWISE            ;  RETU
   ENDCASE
ENDDO
*: FIM: FOC.PRG
